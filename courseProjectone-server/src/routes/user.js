...
router.post('/users', async (req, res) => {
    ...
    try {
        ...
        console.log(error);
        if (error.code === 11000) { // Mongo error code
            res.status(400).send({ error: 'EMAIL_EXISTS' });
        }
        res.status(400).send({ error: error.message || 'Authentication Error!' });
    }
...
router.patch('/users/me', auth, async (req, res) => {
    ...
    try {
        ...
        req.user = await User.findByIdAndUpdate(req.user.id, req.body, { new: true, runValidators: true })
